...
#set how the program shuts down 1 for program shutdown 2 for linux shutdown
endMode = 	1
...
def drawMotion(pos, radius, deg, mag):
	...
	print(int(math.cos(math.radians(deg)) * length))
	print(int(math.sin(math.radians(deg))* length))
...
def manualMode():
	...
	while loopQuit == False:
		...
		mag = int(axis[3] * -100)	
		outpt = str(angle) + "," + str(mag)	+ "," 
		#Get button inputs
		for i in range(8,12):
			buttonInput = joystick.get_button(i)
			outpt = outpt + str(buttonInput) + ","		
